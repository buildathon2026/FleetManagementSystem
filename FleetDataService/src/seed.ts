...
const truckDetails: Record<string, { year: number; make: string; model: string; vin: string }> = {
...
function randomDate(startMonth: number, endMonth: number, year = 2026): string {
...
export function seedDatabase(): void {
  ...
  const insertExpense = db.prepare(
    'INSERT INTO expenses (truck_id, date, amount, category, description, doc_ref) VALUES (?, ?, ?, ?, ?, ?)'
  );
  ...
  for (const truckId of trucks) {
    ...
    for (let j = 0; j < fuelCount; j++) {
      insertExpense.run(
        ...
        randomFloat(250, 650),
        'fuel',
        fuelDescriptions[randomBetween(0, fuelDescriptions.length - 1)],
        `DOC-FUEL-${truckId}-${j}`
      );
    }
    ...
    for (let j = 0; j < partsCount; j++) {
      insertExpense.run(
        ...
        randomFloat(50, 2500),
        'parts',
        partsDescriptions[randomBetween(0, partsDescriptions.length - 1)],
        `DOC-PARTS-${truckId}-${j}`
      );
    }
    ...
    for (let j = 0; j < laborCount; j++) {
      insertExpense.run(
        truckId,
        randomDate(1, 6),
        randomFloat(200, 1800),
        'labor',
        laborDescriptions[randomBetween(0, laborDescriptions.length - 1)],
        `DOC-LABOR-${truckId}-${j}`
      );
    }
    ...
    for (let q = 1; q <= 2; q++) {
      ...
      insertExpense.run(
        truckId,
        randomDate(month, month),
        randomFloat(800, 1500),
        'insurance',
    ...
    insertExpense.run(
      truckId,
      randomDate(1, 3),
      randomFloat(1200, 2200),
      'registration',
      'Annual registration renewal',
      `DOC-REG-${truckId}-2026`
    );
    ...
    for (let j = 0; j < tollCount; j++) {
      insertExpense.run(
        ...
        randomFloat(5, 45),
        'toll',
        tollDescriptions[randomBetween(0, tollDescriptions.length - 1)],
        `DOC-TOLL-${truckId}-${j}`
      );
    }
  }
  ...
  const insertRevenue = db.prepare(
    'INSERT INTO revenue (truck_id, date, amount, load_id, description) VALUES (?, ?, ?, ?, ?)'
  );
  ...
  for (const truckId of trucks) {
    ...
    for (let j = 0; j < loadCount; j++) {
      insertRevenue.run(
        truckId,
        randomDate(1, 6),
        randomFloat(1500, 8500),
        generateLoadId(),
        routes[randomBetween(0, routes.length - 1)]
      );
    }
  }
  ...
  const insertDoc = db.prepare(
    'INSERT INTO documents (id, entity_id, doc_type, date, summary, content, active) VALUES (?, ?, ?, ?, ?, ?, ?)'
  );
  ...
  for (const truckId of trucks) {
    ...
    insertDoc.run(
      ...
      'registration',
      regExpiry,
      `Vehicle registration for ${truckId} - ${detail.year} ${detail.make} ${detail.model}`,
      JSON.stringify({ vin: detail.vin, state: 'TX', plate: `TX-${randomBetween(100, 999)}-${randomBetween(1000, 9999)}`, expiry: regExpiry }),
      1
    );
    ...
    insertDoc.run(
      ...
      'insurance',
      insExpiry,
      `Commercial auto insurance policy for ${truckId}`,
      JSON.stringify({ provider: 'Progressive Commercial', policy: `POL-${randomBetween(100000, 999999)}`, expiry: insExpiry, coverage: 'Full' }),
      1
    );
    ...
    insertDoc.run(
      ...
      'title',
      `${detail.year}-06-15`,
      `Certificate of title for ${detail.year} ${detail.make} ${detail.model}`,
      JSON.stringify({ vin: detail.vin, state: 'TX', owner: 'Fleet Operations LLC' }),
      1
    );
    ...
    insertDoc.run(
      ...
      'inspection',
      inspDate,
      `Annual DOT inspection - ${truckId} passed`,
      JSON.stringify({ result: 'PASS', inspector: 'DOT Station 42', next_due: '2027-04-01' }),
      1
    );
    ...
    insertDoc.run(
      ...
      'tax_form',
      '2026-01-15',
      `HVUT Form 2290 for ${truckId} - Tax Year 2025-2026`,
      JSON.stringify({ form: '2290', tax_year: '2025-2026', amount_paid: randomFloat(500, 600) }),
      1
    );
    ...
    for (let j = 0; j < maintCount; j++) {
      insertDoc.run(
        ...
        'maintenance',
        randomDate(1, 6),
        `Maintenance record - ${['Oil change', 'Brake service', 'Tire replacement', 'Engine diagnostics'][randomBetween(0, 3)]}`,
        JSON.stringify({ shop: 'Fleet Service Center', mileage: randomBetween(100000, 500000), items: ['Service completed'] }),
        1
      );
    }
    ...
    insertDoc.run(
      ...
      'settlement',
      randomDate(4, 5),
      `Weekly settlement statement for ${truckId}`,
      JSON.stringify({ period: 'Week 18, 2026', gross: randomFloat(5000, 12000), deductions: randomFloat(1000, 3000) }),
      1
    );

    // Fuel receipts
    insertDoc.run(
      `DOC-FUELRCPT-${truckId}`,
      truckId,
      'fuel_receipt',
      randomDate(5, 6),
      `Fuel receipt - Pilot Travel Center`,
      JSON.stringify({ gallons: randomFloat(100, 200), price_per_gallon: randomFloat(3.5, 4.2), location: 'Pilot #4521, Dallas TX' }),
      1
    );

    // Toll receipt
    insertDoc.run(
      `DOC-TOLLRCPT-${truckId}`,
      truckId,
      'toll_receipt',
      randomDate(4, 6),
      `EZPass monthly toll statement`,
      JSON.stringify({ month: 'May 2026', total_tolls: randomFloat(150, 450), transactions: randomBetween(15, 40) }),
      1
    );
  }
